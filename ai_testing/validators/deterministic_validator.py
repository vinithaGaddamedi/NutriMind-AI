...
from agents.infrastructure.schemas.validator_schemas import ValidationResult, ConstraintViolation
...
class DeterministicValidator:
    ...
    def validate_allergy_constraint(self, response_text: str, allergies: List[str]) -> tuple[bool, List[str]]:
        violations = []
        response_lower = response_text.lower()
        for allergy in allergies:
            allergy_lower = allergy.lower()
            # Reuse the allergy keyword mapping or fallback to word boundaries
            patterns = ALLERGY_KEYWORD_MAP.get(allergy_lower, [rf"\b{allergy_lower}\b"])
            for pattern in patterns:
                match = re.search(pattern, response_lower)
                if match:
                    violations.append(f"Allergen keyword detected: {match.group()}")
